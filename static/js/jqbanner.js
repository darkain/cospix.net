...
jQuery(document).ready(function() {
	...
	//preload this and next images
	jqb_show( first.find('img') );
	jqb_show( first.next().find('img') );

	$('.jqb-slide').find('.jqb-text').hide()	;

	jqb_vTotal = $('.jqb-slides').children().size() -1;
...
function jqb_fnDone(){ jqb_vBusy = false; }

function jqb_show(selection){
	$(selection).each(function(index,item){
		var img = $(item);
		var src = img.attr('src');
		if (typeof src == 'undefined') img.attr('src', img.data('src'));
		if (typeof src != 'undefined'  &&  src.indexOf('blank.gif') >= 0) img.attr('src', img.data('src'));
		img.parents('.jqb-slide').show();
	});
}
	...
	if(!jqb_vBusy){
		...
		$('#jqb-object').find('.jqb-slide').each(function(i) {
			if (jqb_vCurrent == i){
				...
				//preload this and next images
				jqb_show( $(this).find('img') );
				jqb_show( $(this).next().find('img') );
			}
