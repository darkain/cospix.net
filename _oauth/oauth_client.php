...
*/
	...
	Function GetDialogURL(&$url, $redirect_uri = '', $state = '')
	{
		...
		$url = str_replace(
			'{REDIRECT_URI}', rawurlencode($redirect_uri), str_replace(
			'{STATE}', rawurlencode($state), str_replace(
			'{CLIENT_ID}', rawurlencode($this->client_id), str_replace(
			'{API_KEY}', rawurlencode($this->api_key), str_replace(
		...
		if(IsSet($oauth))
		{
			...
			else
			{
				...
				&& count($parameters))
				{
					...
					{
						$url .= ($first ? '?' : '&').rawurlencode($parameter).'='.rawurlencode($value);
						$first = false;
					}
					$parameters = array();
				}
			...
			if(count($parameters))
			{
				switch($type)
				{
					...
					default:
						...
						{
							$url .= ($first ? '?' : '&').$name.'='.rawurlencode($value);
							$first = false;
						}
				}
			}
		...
		switch(intval($this->oauth_version))
		{
			...
			case 2:
				...
				if(strcasecmp($this->access_token_type, 'Bearer'))
					$url .= (strcspn($url, '?') < strlen($url) ? '&' : '?').(strlen($this->access_token_parameter) ? $this->access_token_parameter : 'access_token').'='.rawurlencode($this->access_token);
				break;
			...
			case 1:
				...
				if(!IsSet($access_token['authorized']))
				{
					...
					if(!$this->GetRequestTokenURL($url))
						return false;
					$url = str_replace('{SCOPE}', rawurlencode($this->scope), $url);
					if(!$this->GetRedirectURI($redirect_uri))
						return false;
					$oauth = array(
				...
				if(!$one_a)
				{
					if(!$this->GetRedirectURI($redirect_uri))
						return false;
					$url .= '&oauth_callback='.rawurlencode($redirect_uri);
				}
				if($this->debug)
